...
def main_params():
    ...
    range_or_inds = 0                               # ==1: pick a range of stations, ==0: pick individual stations
    ...
    return (super_comp, ramping_const, load_pr_case, inc_h2_demand, first_station, last_station, cap_class, hour, day, station,
            ...
            hhv, h2_convert, k_Double, range_or_inds, stations_to_run)
...
def main_function():
    (super_comp, ramping_const, load_pr_case, inc_h2_demand, first_station, last_station, cap_class, hour, day, station,
     no_station_to_run, ir, capex_B, life_B, fixed_OM_B, p_BC_fixed, p_BE, h_B, ef_B, p_HK_fixed, p_HC_fixed, p_HE, ef_H,
     r_M, k_M, life_M, capex_M, fixed_OM_M, var_OM_M, fuel_M, g_M_min, capex_P, life_P, fixed_OM_P, p_PE, p_WO, cf_W, h2_demand_p,
     hhv, h2_convert, k_Double, range_or_inds, stations_to_run) = main_params()

    (model_dir, load_folder, solar_folder, trans_folder, results_folder, charging_station_folder) = working_directory(super_comp)

    (I, S, T, S_t, S_r) = main_sets(no_station_to_run, cap_class, hour, station, first_station, last_station, range_or_inds, stations_to_run)
...
# %% Define set:
def main_sets(no_station_to_run, cap_class, hour, station, first_station, last_station, range_or_inds, stations_to_run):
...
def model_solve(S, S_r, S_t, T, I, model_dir, results_folder, load_folder, solar_folder, trans_folder,
    ...
    for cs in S_r:
        ...
        def market_clearing(model, s, t):
            if inc_h2_demand == 0:
                ...
                return model.g_B[s, t] + model.g_P[s, t] + model.g_M[s, t] \
                       + model.g_W[s, t] >= d_E[t] + model.d_B[s, t] + model.d_H[s, t]/c_H
        ...
        results_book = xw.Workbook(update_results_folder + 'HDV_model_results for station ' + str(station_no) + '.xlsx')
        ...
        for s in S:
            if len(str(station_no)) == 1:
                station_number[s] = "00" + str(station_no)
            elif len(str(station_no)) == 2:
                station_number[s] = "0" + str(station_no)
            elif len(str(station_no)) == 3:
                station_number[s] = str(station_no)
        ...
        result_sheet_ob.write("A1", "station no")
        result_sheet_ob.write("A2", station_number[0])
        result_sheet_ob.write("B1", "total cost (million $)")
        result_sheet_ob.write("B2", total_cost / 1000000)

        # Write power rating results:
        result_sheet_pr.write("A1", "station no")
        ...
        result_sheet_pr.write("E1", "h2 energy capacity (kg)")
        ...
        result_sheet_tc.write("A1", "station no")
        for item in S:
        ...
        result_sheet_bg.write("A1", "station no")
        for item in S:
        ...
        result_sheet_hg.write("A1", "station no")
        for item in S:
        ...
        result_sheet_mg.write("A1", "station no")
        for item in S:
        ...
        result_sheet_sg.write("A1", "station no")
        for item in S:
        ...
        result_sheet_wg.write("A1", "station no")
        for item in S:
        ...
        result_sheet_bi.write("A1", "station no")
        for item in S:
        ...
        # h2 inflow results:
        result_sheet_hi.write("A1", "station no")
        for item in S:
        ...
        # battery SOC results:
        result_sheet_bs.write("A1", "station no")
        for item in S:
        ...
        # H2 SOC results:
        result_sheet_hs.write("A1", "station no")
        for item in S:
    ...
    # Combine all station results into one file:
    cwd = os.path.dirname(update_results_folder)
    all_files = glob.glob(update_results_folder + "HDV_model_results*.xlsx")
    sheetname1 = 'power rating'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname1)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'power rating all stations.xlsx')

    sheetname2 = 'transmission class'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname2)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'transmission class all stations.xlsx')

    sheetname3 = 'battery gen'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname3)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'battery gen all stations.xlsx')

    sheetname4 = 'h2 gen'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname4)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'h2 gen all stations.xlsx')

    sheetname5 = 'smr gen'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname5)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'smr gen all stations.xlsx')

    sheetname6 = 'solar gen'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname6)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'solar gen all stations.xlsx')

    sheetname7 = 'purchased gen'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname7)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'purchased gen all stations.xlsx')

    sheetname8 = 'total cost'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname8)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'total cost all stations.xlsx')

    sheetname9 = 'battery inflow'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname9)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'battery inflow all stations.xlsx')

    sheetname10 = 'h2 inflow'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname10)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'h2 inflow all stations.xlsx')

    sheetname11 = 'battery SOC'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname11)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'battery SOC all stations.xlsx')

    sheetname12 = 'h2 SOC'
    li = []
    for filename in all_files:
        df = pd.read_excel(filename, sheet_name=sheetname12)
        li.append(df)
    frame = pd.concat(li, axis=0)
    frame.to_excel(update_results_folder + 'h2 SOC all stations.xlsx')
