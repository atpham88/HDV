...
hour = 8760  # Number of hours
no_day = 365  # Number of days in a typical year
...
load_data_annual = pd.DataFrame(np.tile(load_data_raw_2, no_day))
...
f_P_annual = pd.DataFrame(np.tile(f_P_24, no_day))
...
p_WE_annual = pd.DataFrame(np.tile(p_WE_24, no_day))
